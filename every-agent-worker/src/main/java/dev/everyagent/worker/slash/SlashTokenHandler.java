 ...
 * 逐段调用 {@link #resolve(String)} 按 kind 分发——核心层对 kind 与 payload 内容零感知,
 * 新增来源只需加一个 {@code @Component SlashTokenResolver},核心零改动(对齐 old 可插拔哲学)。
 *
 * <p>兜底:解析失败或未知 kind 时 {@link #resolve(String)} 返回原 opaque 串(与老项目
 * {@code resolveTokenReplacement} 保留未知 token 的行为一致),绝不误删半成品 token。
 */
@Component
public class SlashTokenHandler {
    ...
    /**
     * 解析单个 opaque token 为提交给 AI 的文本:
     * <ul>
     *   <li>非 opaque 格式 / 解析失败 → 原样返回(兜底保留);</li>
     *   <li>未知 kind → 原样返回;</li>
     *   <li>已知 kind → resolver 输出(返回空串即「从 AI 上下文剥离」,如 git.auto_sync)。</li>
     * </ul>
     */
