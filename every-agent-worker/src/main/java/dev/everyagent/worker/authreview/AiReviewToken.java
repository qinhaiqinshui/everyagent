...
/**
 * 「/AI 审议」斜杠能力 capsule token 的 worker 侧实现(仿 {@code modelpool.ModelPoolToken}:
 * opaque token 三要素——固定 kind、构造、命中扫描)。
 *
 * <p>职责:
 * <ul>
 *   <li>固定 {@link #KIND} = {@code ai.review};</li>
 ...
 * (从 AI 上下文剥离)由 {@code AiReviewSlashResolver} 接管:本 kind 解析为 {@code ""},
 * token 被清空、不注入模型上下文。真正的「授权弹窗改 AI 审议」由
 * {@code PermissionGate} 按任务级 {@code TaskEntry.aiReview} 分派(步骤 6 实现)。
 *
 * <p>本次任务有效:选中 /AI 审议 即开启任务级开关(写入任务 meta.json 落盘),
 * 之后本任务所有轮次持续生效、再运行仍保持,直至 ✕ 取消。
 */
public final class AiReviewToken {

    /** AI 审议 token 的固定 kind(提交解析 / 胶囊回显共用)。 */
    public static final String KIND = "ai.review";

    private AiReviewToken() {
    }

    /** 构造 AI 审议 opaque token(select 直接返回该串 → 底部渲染胶囊)。 */
    public static String buildToken() {
        return SlashTokenEncoder.buildToken(
                KIND,
                "AI 审议",
                "授权弹窗改为 AI 安全审议（本任务有效）",
                Json.obj().put("enabled", true));
    }

    /**
     * 判断文本是否包含「AI 审议」标记(命中 {@code ai.review} 且 enabled=true)。
    ...
    /**
     * 扫描文本命中 {@code ai.review} token 时返回其 {@code enabled} 值;
