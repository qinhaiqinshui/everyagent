...
/**
 * 「/网络」命令来源(仿 {@code modelpool.ModelPoolSlashProvider}):
 * 用户选择后把「网络」胶囊(bottom 底部渲染)挂到本任务,业务 onSelect/onCancel
 * 用 taskId 读写任务级开关 {@code TaskEntry.networkAllowed} 并随 meta.json 落盘。
 *
 * <p>选中即开启<b>任务级</b>「允许访问网络」开关并落盘——{@code CommandExecutor}
...
public class NetworkSlashProvider {
    ...
    private static final String NETWORK_GROUP = "网络";
    ...
    /** 返回 `/网络` 候选项(选中即构造自包含 opaque 串 → 底部胶囊渲染,可取消)。 */
    private List<SlashCommandItem> items() {
        String subtitle = "允许本任务访问网络（默认关闭，本任务有效，底部可取消）";
        ...
        SlashSelectHandler selectHandler = (item, taskId) -> {
            if (taskId != null && !taskId.isEmpty()) {
                ...
                if (t != null) {
                    t.networkAllowed = true; // 注册方写自己的业务标记
        ...
        SlashCancelHandler cancelHandler = (item, token, taskId) -> {
            if (taskId != null && !taskId.isEmpty()) {
                TaskEntry t = taskManager.runningTask(taskId);
                if (t != null) {
                    t.networkAllowed = false; // 注册方删自己的业务标记
                    t.persist();
                }
            }
        };
